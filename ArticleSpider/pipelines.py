...
# @Description : 这段代码是一个用于爬取文章信息的Scrapy项目的管道（pipeline）集合。
# 其中包括了将爬取结果以JSON格式保存到文件、同步写入MySQL数据库、异步写入MySQL数据库（使用Twisted框架）、以及将数据写入Elasticsearch的功能。
# 具体而言，它实现了数据的持久化存储，包括保存为JSON文件、存入MySQL数据库（支持同步和异步两种方式），以及将数据索引到Elasticsearch中，以便进行检索和分析。
# 此外，还包含了处理文章图片下载的管道，将图片保存并记录路径。
import codecs
import json
