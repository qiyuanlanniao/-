...
# @Description : 这段代码实现了一个使用 Scrapy 和 Selenium 模拟登录的爬虫，用于爬取博客园（cnblogs.com）上的文章信息。
# 通过登录获取 Cookies，然后在请求中附加这些 Cookies，实现登录状态下的页面爬取。使用 ItemLoader 对数据进行加载和预处理，
# 包括提取文章标题、发布日期、内容、标签等信息，并在解析文章详情页时，通过发送额外的请求获取文章的点赞数、收藏数和评论数等信息。
# 最终将处理好的数据存储在自定义的 Item 对象中。
